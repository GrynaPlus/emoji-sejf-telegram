// Inicjalizacja Supabase z Twoimi danymi
const supabaseUrl = 'https://bpymvghohhpjoaphchqh.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJweW12Z2hvaGhwam9hcGhjaHFoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDIwNDUxNzEsImV4cCI6MjA1NzYyMTE3MX0.o3i5pUoWSRsmumHKavWICmbELTyyJxiTzzlKvjTElFM';
...
async function saveUser(username, level) {
  const { data, error } = await supabase
    .from('users')
    .insert([{ username: username, level: level }]);
  
  if (error) {
    console.error('Błąd zapisu użytkownika:', error);
  } else {
    console.log('Dane użytkownika zapisane:', data);
  }
}
...
async function updateUserLevel(username, level) {
  ...
  if (error) {
    ...
    console.log('Poziom użytkownika zaktualizowany:', data);
  }
}
...
function openSafeAnimation() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      // Reklama In-App Interstitial co 3 poziomy
      if (currentLevel % 3 === 0) {
        show_9087151({
          type: 'inApp',
          inAppSettings: { 
...
function nextLevel() {
  currentLevel++;
  // Aktualizacja poziomu użytkownika w Supabase
  updateUserLevel(username, currentLevel);
...
usernameSubmit.addEventListener("click", function() {
  ...
  if (name) {
    ...
    updateUserDisplay();
    // Zapisz nowego użytkownika do Supabase z początkowym poziomem = 1
    saveUser(username, currentLevel || 1);
    initGame();
  } else {
